...
q4= Rotation.from_euler("xyz", [0, 0, 60], degrees=True) * q3

rod1  = Rod(mass=1, inertia=np.eye(3), length=LENGTH, state=RodState(r=np.array([ OFFSET*cos(0*pi/2+pi/4),  OFFSET*sin(0*pi/2+pi/4), 0]), q=q1), fixed = True)
